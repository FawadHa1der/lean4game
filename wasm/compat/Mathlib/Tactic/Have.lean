...
wasm/compat: `Mathlib.Tactic.Have` as of mathlib4 tag v4.23.0, compiled
under its real module name (see wasm/compat/README.md). The module is
present upstream but EXCLUDED from the essential Mathlib pack this build
ships; games TEACH the goal-creating `have h : t` / `suffices h : t`
syntax (players type it live), so the extension must exist in the game
environment. Body identical to the copy games vendored as
`Game/Tactic/MathlibHave.lean`.
-/
