/-
wasm/compat: verbatim copy of mathlib4 de3a9cf `Mathlib.Tactic.Cases` (Apache-2.0; see
wasm/compat/README.md). The essential pack excludes this leaf. This is the FULL upstream file
(`ElimApp.evalNames` + the `cases'` / `induction'` elaborators), replacing the v4.23.0 evalNames-only
subset: RealAnalysisGame and NumberTheoryGame teach `cases'` / `induction'` (players type them live).
It uses `import all Lean.Elab.Tactic.Induction`, so it must be compiled against the fat lib-tree.
-/
/-
