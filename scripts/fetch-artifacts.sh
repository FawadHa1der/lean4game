...
python3 -c "import json;[print(t['file'],t['sha256'],t['bytes'],t['class']) for t in json.load(open('$BUNDLE'))['tarballs']]" | while read -r file digest bytes cls; do
  ...
  rm -rf "$PUB/$cls"; mkdir -p "$PUB"
  tar -xf "$local_tar" -C "$PUB"
  echo "  extracted into client/public/$cls ($(find "$PUB/$cls" -type f | wc -l | tr -d ' ') files)"
done
